package com.real.doctor.realdoc.widget.timepicker;
...
public class CustomDatePicker {
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy年MM月dd日 HH:mm") && isValidDate(endDate, "yyyy年MM月dd日 HH:mm")) {
            canAccess = true;
    ...
    public void show(String time) {
        if (canAccess) {
            if (isValidDate(time, "yyyy年MM月dd日")) {
    ...
    public void setSelectedTime(String time) {
        if (canAccess) {
            ...
            String mYear = str[0].substring(0, 4); //取年
            String mMonth = str[0].substring(str[0].indexOf("年") + 1, str[0].indexOf("月")); //取月
            String mDay = str[0].substring(str[0].indexOf("月") + 1, str[0].indexOf("日")); //取日

            year_pv.setSelected(mYear);
            selectedCalender.set(Calendar.YEAR, Integer.parseInt(mYear));
            ...
            month_pv.setData(month);
            month_pv.setSelected(mMonth);
            selectedCalender.set(Calendar.MONTH, Integer.parseInt(mMonth) - 1);
            executeAnimator(month_pv);
            ...
            day_pv.setData(day);
            day_pv.setSelected(mDay);
            selectedCalender.set(Calendar.DAY_OF_MONTH, Integer.parseInt(mDay));
            executeAnimator(day_pv);
